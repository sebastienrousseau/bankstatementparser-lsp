...
# Releasing bankstatementparser-lsp
...
bankstatementparser-lsp tracks
[`bankstatementparser`](https://github.com/sebastienrousseau/bankstatementparser) version-for-
version: when bankstatementparser ships `0.0.X`, bankstatementparser-lsp ships a matching
`0.0.X`. This keeps the editor surface aligned with the core library's
...
for LSP-only bug fixes that don't require a bankstatementparser bump.

## What merits a release

Cut a new version when there is user-visible change to ship - bug fixes,
security or dependency patches, new editor features (diagnostics,
completion entries, code actions), or documentation that ships in the
package.
...
5. The version is identical in `pyproject.toml` and
   `bankstatementparser_lsp/__init__.py` (enforced by `scripts/verify_versions.py`).

## Cutting the release

1. Bump the version in `pyproject.toml` and `bankstatementparser_lsp/__init__.py`
   and add the `CHANGELOG.md` section in a single PR.
2. Merge the PR to `main` once CI is green.
3. Push a signed tag:

   ```bash
   git tag -s vX.Y.Z -m "bankstatementparser-lsp vX.Y.Z" <merge-commit>
...
- Confirm the version is live on
  [PyPI](https://pypi.org/project/bankstatementparser-lsp/) and the GitHub release is
  published (not draft).
- Verify a clean install: `pip install bankstatementparser-lsp==X.Y.Z`.
...
- **PyPI trusted publisher** (`release.yml`): configured at
  ...
  set is `repo:sebastienrousseau/bankstatementparser-lsp:environment:pypi` with
