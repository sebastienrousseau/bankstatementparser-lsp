...
# bankstatementparser-lsp Governance

This document describes how bankstatementparser-lsp is run, how decisions are made,
...
bankstatementparser-lsp is the Language Server Protocol surface for the
[`bankstatementparser`](https://github.com/sebastienrousseau/bankstatementparser)
library. It surfaces live diagnostics in any LSP-capable editor when
authoring MT940 bank-statement files (`.mt940` / `.sta`). Changes are
...
- **Significant changes** (new public APIs, breaking changes, new
  dependencies, new LSP features) need explicit approval from a
...
Releases follow [`RELEASING.md`](RELEASING.md). bankstatementparser-lsp tracks
[`bankstatementparser`](https://github.com/sebastienrousseau/bankstatementparser) version-for-
version: when bankstatementparser cuts `0.0.X`, bankstatementparser-lsp ships a matching
...
1. Contribute a few reviewed PRs in an area
   ...
   new diagnostic rules, code actions, examples).
...
bankstatementparser-lsp today has **one** maintainer, which is a real risk for a
...
- **The work is legible:** [`ARCHITECTURE.md`](ARCHITECTURE.md) maps the
  ...
  and every public helper ships a runnable example.
